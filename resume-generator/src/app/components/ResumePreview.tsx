...
export default function ResumePreview({ name, email, phone, address, link, education, experience, skills, projects, showEducation, showExperience, showSkills, showProjects, sectionOrder }: ResumePreviewProps) {
  return (
    <div
      ...
      style={{ fontFamily: 'Calibri, "Segoe UI", Roboto, sans-serif' }}
    >
      <h1 className="text-xl md:text-2xl font-bold text-center mb-2 md:mb-4">
        {name || "Firstname Lastname"}
      </h1>
      <p className="text-center mb-4 md:mb-6 text-sm text-gray-600">
            ...
            <div key="education" className="mb-6">
              ...
              {education.map((edu, idx) => (
                <div key={idx} className="mb-4">
                  <div className="flex flex-col md:flex-row md:justify-between font-semibold">
                  ...
                  {edu.comment && <div className="text-sm mt-1">{edu.comment}</div>}
                </div>
              ))}
            </div>
          );
        ...
        if (section === "Skills" && showSkills) {
          return (
            <div key="skills" className="mb-6">
              ...
              <ul className="text-sm grid gap-y-2 list-disc list-inside">
                {skills.map((skill, idx) => (
                  <div key={idx} className="text-sm whitespace-pre-line">{skill.skill}</div>
                ))}
              </ul>
            </div>
          );
            ...
            <div key="projects" className="mb-6">
              ...
              {projects.map((proj, idx) => (
                <div key={idx} className="mb-4">
                  ...
                  <div className="text-sm whitespace-pre-line">{proj.description}</div>
                  {proj.link && (
